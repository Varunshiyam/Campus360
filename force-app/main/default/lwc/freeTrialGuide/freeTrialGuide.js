...
export default class FreeTrialGuide extends LightningElement {
  ...
  async handleCreateTargetOrg() {
    ...
    try {
      ...
      this.provisioningError =
        e.body?.message ||
        e.message ||
        "Request failed. Check Setup > Remote Site Settings for HostedScratch.";
    } finally {
  ...
  get provisioningUrl() {
    const base =
      "https://hosted-scratch.herokuapp.com/launch?template=https://github.com/SFDC-Assets/platformTrial/tree/jan26-trial-experience";
    const email = this.userEmail || "";
    if (email.trim()) {
