...
export default class Complaint extends LightningElement {
    @track searchKey = '';
    @track studentId = '';
    @track studentName = '';
    ...
    handleSuccess(event) {
        this.isVerifying = false;
        this.showToast('Success', 'Complaint registered successfully! ID: ' + event.detail.id, 'success');
        this.handleReset();
    }

    handleError(event) {
        this.isVerifying = false;
        this.showToast('Error', 'Failed to submit complaint: ' + event.detail.detail, 'error');
    }
